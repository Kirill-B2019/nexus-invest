                            ...
                            <tr>
                                ...
                                <td class="text-end actions-cell">
                                    <div class="table-actions-desktop d-none d-md-block">
                                        <form method="post" action="{{ route('lk.admin.news-feed.destroy', ['newsFeedItem' => $item->id]) }}" class="d-inline" data-swal-confirm="{{ __('Удалить статью с сайта? Картинка также будет удалена.') }}" data-swal-title="{{ __('Подтверждение') }}">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    <div class="table-actions-mobile d-md-none dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            <form method="post" action="{{ route('lk.admin.news-feed.destroy', ['newsFeedItem' => $item->id]) }}" class="dropdown-item p-0" data-swal-confirm="{{ __('Удалить статью с сайта? Картинка также будет удалена.') }}" data-swal-title="{{ __('Подтверждение') }}">
                                                @csrf
                                                @method('DELETE')
